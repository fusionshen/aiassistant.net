...
namespace AI_Assistant_Win.Utils
{
    public class HybridCircularDetector
    {
          ...
          DetectionParams param = null)
        {
            ...
            // 阶段3：几何校正与最外层类圆轮廓查找
            var (dominantContour, area) = RefineCircular(mainContour);
            var loss = Math.Abs(area - yoloPoints.Count) / yoloPoints.Count;
            if (loss > 0.02)
            {
                return (null, yoloPoints.Count);
            }
            else
            {
        ...
        private static Mat CreateYoloRegionMask(List<PointF> points, Size size, int margin)
        {
            ...
            CvInvoke.Imwrite("Circle_1_Yolo-Mask.png", mask);
            return mask;
        }
        ...
        private static VectorOfPoint ExtractDominantContour(Mat image, Mat mask, DetectionParams param)
        {
            ...
            CvInvoke.Imwrite("Circle_3-Morphology.png", processed);
