        ...
        private void AvatarOriginImage_Click(object sender, System.EventArgs e)
        {
            AntdUI.Preview.open(new AntdUI.Preview.Config(form, avatarOriginImage.Image)
            {
                ...
                OnBtns = (id, config) =>
                {
                    ...
                    {
                        case "download":
                            var prefixes = string.IsNullOrEmpty(tempCircularAreaResult.TestNo) ? string.Empty : $"{tempCircularAreaResult.TestNo}_";
                            prefixes = string.IsNullOrEmpty(tempCircularAreaResult.Position) ? prefixes : $"{prefixes}_{tempCircularAreaResult.Position}_";
                            // 弹出文件保存对话框
                            SaveFileDialog saveFileDialog = new()
                            {
                                //Filter = "JPEG Image Files|*.jpg;*.jpeg|All Files|*.*",
                                //DefaultExt = "jpg",
                                FileName = $"{prefixes}圆片面积检测_原图_{DateTime.Now:yyyyMMddHHmmssfff}.jpg",
                    ...
                    switch (id)
                    {
                        case "download":
                            ...
                            SaveFileDialog saveFileDialog = new()
                            {
                                //Filter = "JPEG Image Files|*.jpg;*.jpeg|All Files|*.*",
                                //DefaultExt = "jpg",
                                FileName = $"{prefixes}圆片面积检测_渲染图_{DateTime.Now:yyyyMMddHHmmssfff}.jpg",
                                Title = LocalizeHelper.CHOOSE_THE_LOCATION
                            };
                            if (saveFileDialog.ShowDialog() == DialogResult.OK)
                            {
